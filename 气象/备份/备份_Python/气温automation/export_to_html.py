...
scraped_path = "D:/文档/Python/气温表格图片HTML/scraped_data.json"

# ******** ******** ******** ******** 读取文件 ******** ******** ******** ********
workbook = openpyxl.load_workbook(path) #对应某个bendi.xlsx文件
...
def write_into_station_list():
    ...
    workbook.save(path) #save the file
...
for o in sortedArr:
    for k in o:
        print(o[k], type(o[k]))
        if isinstance(o[k], float) and math.isinf(o[k]): # 清空 -999的值，这里好像有问题，后面再看(检查代码后，不清空 -999更好，可以检查异常站数据)
            o[k] = ''
...
# ******** ******** ******** ******** 运行 ******** ******** ******** ********
write_into_station_list() # 1.把爬取结果写入 station list 文件
