...
let resultArr=[];
for(let i=0;i<arrOfStations.length;i++){let tempStationData={'index':i,'ifRecorded':false,'dataObj':{}};resultArr.push(tempStationData);for(let j=0;j<arrOfObj.length;j++){if(arrOfObj[j]['Аймаг нэр'].trim()===arrOfStations[i]['aimag'].trim()&&arrOfObj[j]['Сумын нэр'].trim()===arrOfStations[i]['cym'].trim()){resultArr[i]['dataObj']['min']=arrOfObj[j]['Хамгийн бага температур'];resultArr[i]['dataObj']['max']=arrOfObj[j]['Хамгийн их температур'];resultArr[i]['ifRecorded']=true}}}
let resultStr='';
for(let i=0;i<resultArr.length;i++){if(i<resultArr.length-1){if(resultArr[i]['ifRecorded']){resultStr+=resultArr[i]['dataObj']['min']+'\t'+resultArr[i]['dataObj']['max']+'\n'}else{resultStr+='无\t无\n'}}else{if(resultArr[i]['ifRecorded']){resultStr+=resultArr[i]['dataObj']['min']+'\t'+resultArr[i]['dataObj']['max']}else{resultStr+='无\t无'}}}
